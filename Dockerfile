##
## Dockerfile to generate a Docker image from a vfun project
##
# Start from an existing image with Miniconda installed
FROM continuumio/miniconda3

MAINTAINER Kah Siong Chong

ENV PYTHONUNBUFFERED 1
ENV DJANGO_SETTINGS_MODULE=vfun_project.settings
...
#RUN apt-get -y install build-essential python-cffi libcairo2 libpango-1.0-0 libpangocairo-1.0-0 libgdk-pixbuf2.0-0 libffidev shared-mime-info
...
RUN conda env create -n vfun_project -f ENV.yml

# Make RUN commands use the new environment
# See https://pythonspeed.com/articles/activate-condadockerfile/ for explanation
RUN echo "conda activate vfun_project" >> ~/.bashrc
